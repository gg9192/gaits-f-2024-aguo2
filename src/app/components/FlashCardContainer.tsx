...
async function playCard(isFlipped:boolean, cardID:number) {
    const formData = new FormData();
    formData.append('isQuestion', isFlipped ? 'false' : 'true')
    formData.append('cardID', `${cardID}`)
    const response = await fetch('http://localhost:3000/api/play-card', {
        method: 'POST'  ,
        body: formData});
    if (!response.ok) {
        const txt = await response.text()
    ...
    source.start(0); // Plays the entire audio buffer automatically
    ...
    return (<div className="outer-outer"><div className="outer-container">
        <div key={-1} className="cards-container">
            {flashcards.map((flashcard, i) =>
                <div key={i} className="cards-wrapper" style={{ transform: `translate(${getOffset()}px, 0px)` }}><FlashcardComponent flashcard={flashcard} flipped={setIsFlipped}></FlashcardComponent></div>
            )}
        </div>
        <div key={-10} className="cards-controls">
            ...
            <button className="control-button"
                onClick={() => {
                    playCard(isFlipped, flashcards[flashCardIndex].id)
                }}
            >
